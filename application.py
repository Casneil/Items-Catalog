...
def login_required(f):
    ...
    def wrap(*args, **kwargs):
        if "username" in login_session:
            ...
            return redirect(url_fir('/glogin'))   
    return wrap 
...
def showItems(mall_id):
    ...
    # creator = getUserInfo(mall.user_id)
...
def newItem(mall_id): 
    ...
    if newItem.user_id != login_session['username']:
        return "<script>function myFunction() {alert('You are not authorized to add to this Shop. Please create your own Shop in order to add items.');}</script><body onload='myFunction()'>" 
    # global newItem
    if request.method == 'POST':
        newItem = Items(name=request.form['name'], description=request.form\
            ['description'], price=request.form['price'], mall_id=mall_id,\
...
@app.route('/item/<int:mall_id>/item/<int:item_id>/edit', methods=['GET', 'POST'])
@login_required
def editedItem(mall_id, item_id):
    editedItem = session.query(Items).filter_by(id=item_id).one()    
    shops = session.query(Mall).filter_by(id=mall_id).all()
    if editedItem.user_id != login_session['username']:
        flash("Cannot edit items you did not created")
        return redirect(url_for('showItems', mall_id=mall_id))
    if request.method == 'POST':
        ...
        return render_template('editItem.html',item=editedItem,item_id=item_id,\
            mall_id=mall_id)


@app.route('/item/<int:mall_id>/item/<int:item_id>/delete', methods= ['GET','POST']) 
@login_required
def deleteItem(mall_id, item_id):
    ...
    if itemToDelete.user_id != login_session['username']:
          flash("Cannot delete items you did not created")
          return redirect(url_for('showItems', mall_id=mall_id))
    if request.method == 'POST':
        ...
        return render_template('deleteItem.html', title='Delete Item',\
         itemToDelete=itemToDelete, mall_id=mall_id, item_id=item_id)    
